...
const pageStyle = {
  ...
  background:"linear-gradient(160deg,#f0f0f5 0%,#e8e8f0 100%)",
  position:"relative",
  overflowX:"hidden",
...
export default function App(){
  ...
  const TaskRow = ({t,showArea})=>(
    <div style={{display:"flex",alignItems:"center",gap:14,padding:"15px 18px",minHeight:56}}>
      ...
      <div style={{flex:1,minWidth:0}} onClick={()=>openDetail(t.id)}>
        ...
        {t.desc&&!t.done&&<div style={{fontSize:12,color:"#8E8E93",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.desc}</div>}
        <div style={{display:"flex",gap:8,marginTop:3,alignItems:"center",flexWrap:"wrap"}}>
          {showArea&&<span style={{fontSize:11,fontWeight:600,color:ACC}}>{getArea(areas,t.area).label}</span>}
          {t.time&&!t.done&&<span style={{fontSize:11,color:"#8E8E93",fontWeight:500}}>{fmt(t.time)}{t.dur?` · ${t.dur}m`:""}</span>}
      ...
      <span onClick={()=>openDetail(t.id)} style={{color:"#C7C7CC",fontSize:16,cursor:"pointer"}}>›</span>
    </div>
  );
  ...
  const TaskDetailSheet = ()=>{
    ...
    const a = getArea(areas, editForm.area);
    return(
      <Sheet onClose={closeDetail} tall>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div style={{fontSize:20,fontWeight:800,color:"#1C1C1E",letterSpacing:-0.5}}>Task Detail</div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{toggle(detailId);closeDetail();}}
              style={{padding:"7px 14px",borderRadius:20,border:`1.5px solid ${editForm.done?"#34C759":ACC}`,
                background:editForm.done?"rgba(52,199,89,0.1)":`${ACC}10`,
                color:editForm.done?"#34C759":ACC,fontSize:13,fontWeight:700,cursor:"pointer"}}>
              {editForm.done?"Undo":"Done"}
            </button>
            <button onClick={()=>delTask(detailId)}
              style={{padding:"7px 14px",borderRadius:20,border:"1.5px solid rgba(232,67,147,0.3)",
                background:"rgba(232,67,147,0.08)",color:"#E84393",fontSize:13,fontWeight:700,cursor:"pointer"}}>
              Delete
            </button>
          </div>
        </div>
        ...
        <input value={editForm.text} onChange={e=>setEditForm(f=>({...f,text:e.target.value}))}
          style={{...inputStyle,marginBottom:12,fontSize:17,fontWeight:600}}/>

        {/* Description */}
        <Lbl>Description</Lbl>
        <textarea value={editForm.desc||""} onChange={e=>setEditForm(f=>({...f,desc:e.target.value}))}
          placeholder="Add more context…"
          style={{...inputStyle,resize:"none",minHeight:80,marginBottom:12,lineHeight:1.5}}/>

        {/* Notes */}
        <Lbl>Notes</Lbl>
        <textarea value={editForm.notes||""} onChange={e=>setEditForm(f=>({...f,notes:e.target.value}))}
          placeholder="Links, references, anything…"
          style={{...inputStyle,resize:"none",minHeight:60,marginBottom:16,lineHeight:1.5}}/>

        {/* Area + Priority */}
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          <div>
            <Lbl>Area</Lbl>
            <select value={editForm.area} onChange={e=>setEditForm(f=>({...f,area:e.target.value}))} style={inputStyle}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          ...
          <div>
            ...
            <input type="number" value={editForm.dur||""} onChange={e=>setEditForm(f=>({...f,dur:+e.target.value}))} style={inputStyle}/>
          </div>
        </div>
    ...
    <div style={pageStyle}>
      ...
      <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
        <div style={{padding:"56px 20px 16px"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <Btn onClick={()=>{setNewT({text:"",area:"inbox",priority:"med",due:"",time:"",dur:30,desc:"",notes:""});setView("new-task");}}>+ Task</Btn>
            </div>
          </div>
        ...
        <div style={{padding:"0 20px"}}>
          ...
          {areas.map(a=>{
            ...
            return(
              <div key={a.id} style={{...glass(0.78,24),borderRadius:20,marginBottom:10,display:"flex",alignItems:"center",gap:14,padding:"16px 18px",position:"relative"}}>
                <div onClick={()=>{setActiveArea(a.id);setView("area");setShowDone(false);}} style={{display:"contents",cursor:"pointer"}}>
          ...
          <div onClick={openNewArea} style={{...glass(0.5,16),borderRadius:20,marginBottom:10,padding:"16px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14,border:"1.5px dashed rgba(108,92,231,0.3)"}}>
            <div style={{width:44,height:44,borderRadius:14,background:`${ACC}10`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,color:`${ACC}80`,flexShrink:0}}>+</div>
      ...
      {showAreaMgr&&(
        <Sheet onClose={()=>setShowAreaMgr(false)} tall>
          <div style={{fontSize:20,fontWeight:800,color:"#1C1C1E",letterSpacing:-0.5,marginBottom:20}}>{editingArea?"Edit area":"New area"}</div>
          <Lbl>Icon</Lbl>
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
            {ICONS.map(ic=>(
              <button key={ic} onClick={()=>setAreaForm(f=>({...f,icon:ic}))}
                style={{width:40,height:40,borderRadius:12,border:`2px solid ${areaForm.icon===ic?ACC:"transparent"}`,background:areaForm.icon===ic?`${ACC}15`:"rgba(255,255,255,0.7)",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{ic}</button>
            ))}
          </div>
          <Lbl>Name</Lbl>
          <input autoFocus placeholder="e.g. Golf, Learning, Travel…" value={areaForm.label} onChange={e=>setAreaForm(f=>({...f,label:e.target.value}))} style={{...inputStyle,marginBottom:12}}/>
          <Lbl>Description</Lbl>
          ...
          <div style={{display:"flex",gap:10}}>
            {editingArea&&editingArea.id!=="inbox"&&(
              <button onClick={()=>{setShowAreaMgr(false);setConfirmDel(editingArea.id);}} style={{flex:1,padding:"14px 0",borderRadius:14,border:"none",background:"rgba(232,67,147,0.1)",color:"#E84393",fontSize:15,fontWeight:700,cursor:"pointer"}}>Delete</button>
            )}
            ...
            <button onClick={saveArea} disabled={!areaForm.label.trim()} style={{flex:2,padding:"14px 0",borderRadius:14,border:"none",background:areaForm.label.trim()?ACC:"rgba(0,0,0,0.06)",color:areaForm.label.trim()?"#fff":"#8E8E93",fontSize:15,fontWeight:700,cursor:"pointer"}}>{editingArea?"Save":"Create area"}</button>
          </div>
        </Sheet>
      )}
  ...
  if(view==="area"&&activeArea){
    ...
    return(
      <div style={pageStyle}>
        <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
          <div style={{padding:"52px 20px 20px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
              <button onClick={()=>{setView("home");setTab("home");}} style={{width:36,height:36,borderRadius:12,...glass(0.85),border:"none",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#1C1C1E"}}>‹</button>
              <div style={{flex:1}}>
              ...
              <button onClick={()=>openEditArea(a)} style={{width:44,height:44,borderRadius:14,background:`${ACC}15`,border:`1px solid ${ACC}28`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,color:ACC,cursor:"pointer"}}>{a.icon}</button>
            </div>
          ...
          <div style={{padding:"0 20px"}}>
            ...
            <button onClick={()=>{setNewT({text:"",area:activeArea,priority:"med",due:"",time:"",dur:30,desc:"",notes:""});setView("new-task");}} style={{width:"100%",padding:"15px",borderRadius:16,border:"none",background:ACC,color:"#fff",fontSize:16,fontWeight:700,cursor:"pointer",letterSpacing:0.2}}>
              + Add to {a.label}
            </button>
            {a.id!=="inbox"&&(
              <button onClick={()=>setConfirmDel(activeArea)} style={{width:"100%",padding:"13px",borderRadius:16,border:"none",background:"transparent",color:"#E84393",fontSize:14,fontWeight:600,cursor:"pointer",marginTop:8}}>Delete this area</button>
            )}
          </div>
        </div>
        <TabBar/>
        ...
        {showAreaMgr&&(
          <Sheet onClose={()=>setShowAreaMgr(false)} tall>
            <div style={{fontSize:20,fontWeight:800,color:"#1C1C1E",marginBottom:20}}>Edit area</div>
            <Lbl>Icon</Lbl>
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
              {ICONS.map(ic=>(<button key={ic} onClick={()=>setAreaForm(f=>({...f,icon:ic}))} style={{width:40,height:40,borderRadius:12,border:`2px solid ${areaForm.icon===ic?ACC:"transparent"}`,background:areaForm.icon===ic?`${ACC}15`:"rgba(255,255,255,0.7)",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{ic}</button>))}
            </div>
            <Lbl>Name</Lbl>
      ...
      <div style={pageStyle}>
        <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
          ...
          <div style={{padding:"0 20px"}}>
            <div style={{...glass(0.78),borderRadius:20,padding:"16px 0 16px 16px",overflow:"hidden"}}>
              <div style={{display:"flex",gap:4}}>
                <div style={{width:52,flexShrink:0,position:"relative",height:TH}}>
                  {HOURS.map(h=>(<div key={h} style={{position:"absolute",top:(h*60-DS)*PX-6,fontSize:11,fontWeight:500,color:"#8E8E93",textAlign:"right",width:48,lineHeight:1}}>{h===12?"noon":h<12?`${h}am`:`${h-12}pm`}</div>))}
                </div>
            ...
            {scheduled.length===0&&<div style={{textAlign:"center",padding:"32px 0",color:"#8E8E93",fontSize:14}}>No tasks scheduled today.<br/>Tap any task to add a time.</div>}
          </div>
        </div>
        <TabBar/>
        <TaskDetailSheet/>
    ...
    <div style={pageStyle}>
      <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
        <div style={{padding:"52px 20px 20px"}}>
          ...
          <div style={{fontSize:14,color:"#8E8E93",marginBottom:20}}>Brain dump or paste an email.</div>
          <div style={{...glass(0.8),borderRadius:14,padding:4,display:"flex",marginBottom:20}}>
            {[{id:"brain",label:"✦ Brain dump"},{id:"email",label:"✉ Email"}].map(m=>(
              <button key={m.id} onClick={()=>{setAiMode(m.id);setAiResult(null);setPending([]);setAiInput("");}}
                style={{flex:1,padding:"9px 0",borderRadius:11,border:"none",background:aiMode===m.id?ACC:"transparent",color:aiMode===m.id?"#fff":"#8E8E93",fontSize:14,fontWeight:700,cursor:"pointer"}}>
                {m.label}
              </button>
            ))}
          </div>
          {!aiResult&&(
            <>
              <textarea autoFocus value={aiInput} onChange={e=>setAiInput(e.target.value)}
                ...
                style={{width:"100%",padding:"16px",borderRadius:16,...glass(0.75),fontSize:16,resize:"none",minHeight:aiMode==="email"?180:130,boxSizing:"border-box",fontFamily:"inherit",outline:"none",color:"#1C1C1E",marginBottom:12,lineHeight:1.5}}/>
              ...
              <button onClick={runAI} disabled={aiLoad||!aiInput.trim()}
                style={{width:"100%",padding:"16px",borderRadius:16,border:"none",background:aiLoad||!aiInput.trim()?"rgba(0,0,0,0.07)":ACC,color:aiLoad||!aiInput.trim()?"#8E8E93":"#fff",fontSize:17,fontWeight:700,cursor:aiLoad?"default":"pointer"}}>
            ...
            <div>
              ...
              <div style={{...glass(0.75),borderRadius:18,overflow:"hidden",marginBottom:14}}>
                {pending.map((p,i)=>{
                  ...
                  return(
                    <div key={p.id}>
                      <div onClick={()=>setPending(ps=>ps.map(x=>x.id===p.id?{...x,sel:!x.sel}:x))} style={{display:"flex",gap:12,padding:"14px 16px",cursor:"pointer",opacity:p.sel?1:0.3}}>
                        <div style={{width:24,height:24,borderRadius:12,border:`2px solid ${p.sel?ACC:"#D1D1D6"}`,background:p.sel?ACC:"transparent",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",marginTop:2}}>
              ...
              <div style={{display:"flex",gap:10}}>
                <button onClick={()=>{setAiResult(null);setPending([]);}} style={{flex:1,padding:"14px 0",borderRadius:14,border:"none",background:"rgba(0,0,0,0.06)",color:"#8E8E93",fontSize:16,fontWeight:600,cursor:"pointer"}}>Back</button>
                <button onClick={addPending} disabled={!pending.some(p=>p.sel)} style={{flex:2,padding:"14px 0",borderRadius:14,border:"none",background:pending.some(p=>p.sel)?ACC:"rgba(0,0,0,0.06)",color:pending.some(p=>p.sel)?"#fff":"#8E8E93",fontSize:16,fontWeight:700,cursor:"pointer"}}>
  ...
  if(view==="new-task") return(
    <div style={{...pageStyle,overflowY:"auto"}}>
      <div style={{position:"relative",padding:"52px 20px 60px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}>
          <button onClick={()=>setView(activeArea?"area":"home")} style={{width:36,height:36,borderRadius:12,...glass(0.85),border:"none",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#1C1C1E"}}>×</button>
          <div style={{fontSize:20,fontWeight:800,color:"#1C1C1E",letterSpacing:-0.5}}>New Task</div>
          <div style={{width:36}}/>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:24}}>
          {["today","tomorrow","later"].map(d=>(
            <button key={d} onClick={()=>setNewDay(d)} style={{padding:"9px 18px",borderRadius:20,border:"none",background:newDay===d?ACC:"rgba(255,255,255,0.7)",color:newDay===d?"#fff":"#1C1C1E",fontSize:15,fontWeight:700,cursor:"pointer",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",textTransform:"capitalize"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:22}}>
          {areas.map(a=>(
            <button key={a.id} onClick={()=>setNewT(n=>({...n,area:a.id}))} style={{padding:"7px 15px",borderRadius:20,border:`1.5px solid ${newT.area===a.id?ACC:"transparent"}`,background:newT.area===a.id?`${ACC}15`:"rgba(255,255,255,0.65)",color:newT.area===a.id?ACC:"#1C1C1E",fontSize:13,fontWeight:600,cursor:"pointer"}}>
              {a.icon} {a.label}
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:22}}>
          {Object.entries(PRI).map(([k,v])=>(
            <button key={k} onClick={()=>setNewT(n=>({...n,priority:k}))} style={{flex:1,padding:"10px 0",borderRadius:14,border:`1.5px solid ${newT.priority===k?v.color:"transparent"}`,background:newT.priority===k?`${v.color}15`:"rgba(255,255,255,0.65)",color:newT.priority===k?v.color:"#8E8E93",fontSize:14,fontWeight:700,cursor:"pointer"}}>
              {v.label}
            </button>
          ))}
        </div>
        <Lbl>Title</Lbl>
        <input autoFocus placeholder="What needs doing?" value={newT.text} onChange={e=>setNewT(n=>({...n,text:e.target.value}))} style={{...inputStyle,marginBottom:10,fontSize:17,fontWeight:600}}/>
        <Lbl>Description</Lbl>
        <textarea placeholder="More context (optional)" value={newT.desc||""} onChange={e=>setNewT(n=>({...n,desc:e.target.value}))} style={{...inputStyle,resize:"none",minHeight:70,marginBottom:10,lineHeight:1.5}}/>
        <Lbl>Notes</Lbl>
        <textarea placeholder="Links, references… (optional)" value={newT.notes||""} onChange={e=>setNewT(n=>({...n,notes:e.target.value}))} style={{...inputStyle,resize:"none",minHeight:50,marginBottom:16,lineHeight:1.5}}/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
          <div><Lbl>Time</Lbl><input type="time" value={newT.time} onChange={e=>setNewT(n=>({...n,time:e.target.value}))} style={inputStyle}/></div>
          <div><Lbl>Duration (min)</Lbl><input type="number" placeholder="30" value={newT.dur||""} onChange={e=>setNewT(n=>({...n,dur:+e.target.value}))} style={inputStyle}/></div>
        </div>
        <Lbl>Due date</Lbl>
        <input type="date" value={newT.due} onChange={e=>setNewT(n=>({...n,due:e.target.value}))} style={{...inputStyle,marginBottom:28}}/>
        <button onClick={addManual} style={{width:"100%",padding:"16px",borderRadius:16,border:"none",background:ACC,color:"#fff",fontSize:17,fontWeight:800,cursor:"pointer",letterSpacing:0.3}}>Create</button>
      </div>
    </div>
  );
...
function Sheet({children,onClose,tall}){
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"flex-end",zIndex:200,backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)"}}
      ...
      <div style={{background:"rgba(245,245,250,0.97)",backdropFilter:"blur(30px)",WebkitBackdropFilter:"blur(30px)",borderRadius:"20px 20px 0 0",padding:"12px 20px 44px",width:"100%",boxSizing:"border-box",maxHeight:tall?"93vh":"auto",overflowY:"auto",paddingBottom:"calc(env(safe-area-inset-bottom) + 44px)"}}>
...
function Btn({children,onClick,ghost}){
  return(
    <button onClick={onClick} style={{padding:"8px 14px",borderRadius:20,border:ghost?`1.5px solid ${ACC}`:"none",background:ghost?"transparent":ACC,color:ghost?ACC:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
      {children}
    </button>
  );
}
  ...
  return(
    <Sheet onClose={onCancel}>
      <div style={{textAlign:"center",padding:"8px 0 20px"}}>
        ...
        <div style={{fontSize:15,color:"#8E8E93",lineHeight:1.5,marginBottom:24}}>All tasks in <strong style={{color:"#1C1C1E"}}>{name}</strong> will move to Inbox. Cannot be undone.</div>
        <div style={{display:"flex",gap:10}}>
...
const inputStyle={width:"100%",padding:"14px 16px",borderRadius:14,border:"1px solid rgba(255,255,255,0.7)",background:"rgba(255,255,255,0.75)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",fontSize:16,fontFamily:"inherit",outline:"none",color:"#1C1C1E",boxSizing:"border-box",display:"block"};