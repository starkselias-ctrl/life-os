...
const IS = {
  ...
  color:"#1C1C1E",boxSizing:"border-box",display:"block",
};
    ...
    <div style={TABBAR}>
      {items.map(v => (
        <button key={v.id} onClick={()=>{ setTab(v.id); setView(v.id); }}
          ...
          <span style={{fontSize:10,fontWeight:tab===v.id?700:500,color:tab===v.id?ACC:"#8E8E93"}}>{v.label}</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <Sheet onClose={onClose} tall>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div style={{fontSize:20,fontWeight:800,color:"#1C1C1E"}}>Task Detail</div>
    ...
    <div style={PAGE}>
      ...
      <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
        <div style={{padding:"56px 20px 16px"}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:24}}>
            {[{v:urgent.length,l:"Urgent",c:"#E84393"},{v:scheduled.length,l:"Scheduled",c:ACC},{v:doneList.length,l:"Done",c:"#34C759"}].map(s=>(
              <div key={s.l} style={{flex:1,...gl(0.8),borderRadius:16,padding:"12px 10px",textAlign:"center"}}>
                <div style={{fontSize:24,fontWeight:800,color:s.c,lineHeight:1}}>{s.v}</div>
                <div style={{fontSize:11,color:"#8E8E93",marginTop:3}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:"0 20px"}}>
              ...
              <div key={a.id} style={{...gl(0.78,24),borderRadius:20,marginBottom:10,overflow:"hidden",position:"relative"}}>
                ...
                <div onClick={()=>setExpandedArea(ea=>ea===a.id?null:a.id)}
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:12,color:"#8E8E93",marginTop:1}}>{a.sub}</div>
          ...
          <div onClick={openNewArea} style={{...gl(0.5,16),borderRadius:20,marginBottom:10,padding:"16px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14,border:"1.5px dashed rgba(108,92,231,0.3)"}}>
        ...
        <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
          <div style={{padding:"52px 20px 20px"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
              <button onClick={()=>{ setView("home"); setTab("home"); }}
                style={{width:36,height:36,borderRadius:12,...gl(0.85),border:"none",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#1C1C1E"}}>‹</button>
              <div style={{flex:1}}>
            ...
            <div style={{...gl(0.8),borderRadius:16,padding:"14px 18px",display:"flex"}}>
              {[{v:open.length,l:"Open"},{v:done.length,l:"Done"},{v:tasks.filter(t=>t.area===activeArea&&t.priority==="high"&&!t.done).length,l:"Urgent",c:"#E84393"}].map((s,i)=>(
                <div key={s.l} style={{flex:1,textAlign:"center",borderRight:i<2?"1px solid rgba(0,0,0,0.06)":"none"}}>
                  ...
                  <div style={{fontSize:11,color:"#8E8E93",marginTop:2}}>{s.l}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{padding:"0 20px"}}>
            {open.length>0 ? (
              ...
              <div style={{...gl(0.7),borderRadius:20,padding:"40px 24px",textAlign:"center",marginBottom:10}}>
            ...
            {done.length>0 && (
              <div style={{...gl(0.65),borderRadius:20,overflow:"hidden",marginBottom:10}}>
                <button onClick={()=>setShowDone(s=>!s)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",background:"none",border:"none",cursor:"pointer",minHeight:44}}>
                  <span style={{fontSize:12,fontWeight:700,color:"#8E8E93",textTransform:"uppercase",letterSpacing:0.8}}>Completed ({done.length})</span>
            ...
            {a.id!=="inbox" && (
              <button onClick={()=>setConfirmDel(activeArea)} style={{width:"100%",padding:"13px",borderRadius:16,border:"none",background:"transparent",color:"#E84393",fontSize:14,fontWeight:600,cursor:"pointer",marginTop:8}}>
                Delete this area
              </button>
            )}
          </div>
      ...
      <div style={PAGE}>
        <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
          <div style={{padding:"52px 20px 16px"}}>
            ...
            <div style={{...gl(0.8),borderRadius:18,padding:"12px 8px",display:"flex",marginBottom:20}}>
              {weekDates.map((d,i)=>{
                const isTd=d.toDateString()===today.toDateString();
                return (
                  <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                    <div style={{fontSize:10,fontWeight:600,color:"#8E8E93",textTransform:"uppercase"}}>{WEEK_DAYS[d.getDay()]}</div>
          ...
          <div style={{padding:"0 20px"}}>
            <div style={{...gl(0.78),borderRadius:20,padding:"16px 0 16px 16px",overflow:"hidden"}}>
                  ...
                  {scheduled.map((t,idx)=>{
                    ...
                    return (
                      <div key={t.id} onClick={()=>openDetail(t.id)} style={{position:"absolute",top,left:0,right:0,height:h,background:ec.bg,borderLeft:`3.5px solid ${ec.border}`,borderRadius:"0 12px 12px 0",padding:"7px 12px",overflow:"hidden",cursor:"pointer"}}>
                        ...
                        <div style={{fontSize:11,color:"#8E8E93"}}>{fmt(t.time)} – {fmt(endStr)}</div>
                      </div>
                    );
                  })}
                </div>
    ...
    <div style={PAGE}>
      <div style={{position:"relative",zIndex:1,paddingBottom:110}}>
        <div style={{padding:"52px 20px 20px"}}>
          ...
          <div style={{...gl(0.8),borderRadius:14,padding:4,display:"flex",marginBottom:20}}>
          ...
          {aiResult && (
            <div>
              ...
              <div style={{...gl(0.75),borderRadius:18,overflow:"hidden",marginBottom:14}}>
                {pending.map((p,i)=>{
      ...
      <div style={{padding:"52px 20px 60px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}>
          <button onClick={()=>setView(activeArea?"area":"home")}
            style={{width:36,height:36,borderRadius:12,...gl(0.85),border:"none",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#1C1C1E"}}>×</button>
          <div style={{fontSize:20,fontWeight:800,color:"#1C1C1E"}}>New Task</div>
          <div style={{width:36}}/>
        </div>
        <div style={{display:"flex",gap:8,marginBottom:24}}>
          {["today","tomorrow","later"].map(d=>(
            <button key={d} onClick={()=>setNewDay(d)}
              style={{padding:"9px 18px",borderRadius:20,border:"none",background:newDay===d?ACC:"rgba(255,255,255,0.7)",color:newDay===d?"#fff":"#1C1C1E",fontSize:15,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:22}}>
          {areas.map(a=>(
            <button key={a.id} onClick={()=>setNewT(n=>({...n,area:a.id}))}
              style={{padding:"7px 15px",borderRadius:20,border:`1.5px solid ${newT.area===a.id?ACC:"transparent"}`,background:newT.area===a.id?`${ACC}15`:"rgba(255,255,255,0.65)",color:newT.area===a.id?ACC:"#1C1C1E",fontSize:13,fontWeight:600,cursor:"pointer"}}>
              {a.icon} {a.label}
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:22}}>
          {Object.entries(PRI).map(([k,v])=>(
            <button key={k} onClick={()=>setNewT(n=>({...n,priority:k}))}
              style={{flex:1,padding:"10px 0",borderRadius:14,border:`1.5px solid ${newT.priority===k?v.color:"transparent"}`,background:newT.priority===k?`${v.color}15`:"rgba(255,255,255,0.65)",color:newT.priority===k?v.color:"#8E8E93",fontSize:14,fontWeight:700,cursor:"pointer"}}>
              {v.label}
            </button>
          ))}
        </div>
